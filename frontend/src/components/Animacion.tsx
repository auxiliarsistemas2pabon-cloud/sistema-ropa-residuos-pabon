...
/** Proveedor global de animación: carga solo las funciones básicas de Framer Motion
 * (el paquete completo pesa mucho más) y respeta la opción "reducir movimiento" del
 * sistema operativo: quien la tenga activada no ve nada moverse. */
...
/** Tarjeta de acceso: entra escalonada y se eleva un poco al pasar el cursor. */
export function Acceso({ to, children }: { to: string; children: ReactNode }) {
  return (
    <EnlaceAnimado to={to} className="acceso" variants={variantesAcceso} whileHover={{ y: -3 }} whileTap={{ scale: 0.98 }}>
      {children}
    </EnlaceAnimado>
  );
}
